...
#define to_imx678(sd) container_of(sd, struct imx678, sd)
...
static void imx678_update_hmax(struct imx678 *imx678)
{
	...
	dev_info(&client->dev, "Update minimum HMAX\n");
						...
						const struct i2c_device_id *id)
{
	...
	imx678->reset_gpio = devm_gpiod_get_optional(dev, "reset2",
							 GPIOD_OUT_HIGH);
	...
	ret = v4l2_async_register_subdev_sensor_common(&imx678->sd);
	if (ret < 0) {
